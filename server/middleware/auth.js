...
exports.checkLoginOrNot = async (req, res, next) => {
  ...
  try {
    const decode = jwt.verify(token, process.env.SECRET_KEY); // console.log("decode",decode);
    let { role } = req.body;
    ...
    } //this req.user is custom req that we are injecting now we can use whereEver token is present // bring in info from DB
  } catch (error) {
